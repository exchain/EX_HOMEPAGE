...
* Date:2013-05-13
* fix : 상세 내용은 history.txt 참고 
* 	  : 2013-05-03 이병직  c[0]=c[0]-a.grid.bDiv.scrollLeft; 삭제 ("rtl" 4번째로 찾을 것.)
* 		--- 셀 resize 시, 정렬선 위치가 어긋남을 수정.
*     : 2013-05-09 이병직 success:function(e,g,h){ 다음에 "function"===typeof this.beforeSuccess&&this.beforeSuccess(e,g,h); 추가. ("script"로 검색 후 success 함수 시작시 추가)
*       --- beforeSuccess 함수 추가 (외부에서 extend 후 동작) - rowNum을 얻음.
*     : 2013-05-10 이병직 a.p.datatype="local";e=ia();R(e,a.grid.bDiv,o,c>1,j); 다음에 "function"===typeof a.p.afterAddJSONDataLocal&&a.p.afterAddJSONDataLocal(e); 추가 ("clientside"로 검색후 R(...) 뒤에 추가)
*       ---  afterAddJSONDataLocal 함수 추가 (외부에서 extend 후 동작) - colSum에 사용. (빠른 연산과 이벤트처리)
*     : 2013-05-21 이병직 ,b(c).jqGrid("setGridWidth",!0===c.p.shrinkToFit?c.p.tblwidth:c.p.width) 부분을 삭제 ("setGroupHeaders" 첫번째 위) ( showHideCol 동작시 내분에서 setGridWidth() 하지 않도록 )
*       --- dynamicCol 연속 사용시  tblwidth 증가현상 회피
*     : 2013-05-30 이병직 {a.p.page=c;e=true}e&&O(); 부분에서 O();를 (jQuery("#load_"+jQuery.jgrid.jqID(a.p.id)).show(),setTimeout(O,100)); 으로 변경
...
"last"+d&&i){a.p.page=c;e=true}e&&(jQuery("#load_"+jQuery.jgrid.jqID(a.p.id)).show(),setTimeout(O,100));return false})}a.p.pginput===true&&b("input.ui-pg-input","#"+j).keypress(function(c){if((c.charCode||c.keyCode||0)===13){if(!o("user"))return false;b(this).val(n(b(this).val(),1));a.p.page=b(this).val()>0?b(this).val():a.p.page;b("#load_"+b.jgrid.jqID(a.p.id)).show();setTimeout(O,100);return false}return this})},ra=function(c){var d,e="",f=a.p.colModel,g=false,h=a.grid.headers[c].el,i="";b("span.ui-grid-ico-sort",h).addClass("ui-state-disabled");b(h).attr("aria-selected","false");if(f[c].lso)if(f[c].lso==="asc"){f[c].lso=
...
b(a.grid.headers[c].el);c=[c.position().left+c.outerWidth()];a.p.direction==="rtl"&&(c[0]=a.p.width-c[0]);c.push(b(a.grid.hDiv).position().top);c.push(b(a.grid.bDiv).offset().top-b(a.grid.hDiv).offset().top+b(a.grid.bDiv).height());return c},la=function(c){var d,e=a.grid.headers,f=b.jgrid.getCellIndex(c);for(d=0;d<e.length;d++)if(c===e[d].el){f=d;break}return f};this.p.id=this.id;-1===b.inArray(a.p.multikey,["shiftKey","altKey","ctrlKey"])&&(a.p.multikey=!1);a.p.keyIndex=
